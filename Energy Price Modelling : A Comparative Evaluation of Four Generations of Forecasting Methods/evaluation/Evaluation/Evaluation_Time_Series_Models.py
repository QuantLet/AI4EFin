...
def evaluate_results(sequence_predictions:dict,
                     EU_countries_dict:dict)->pd.DataFrame:
    """Evaluates the predictions from univariate and multivariate forecasting techniques.
    
    Parameters:
    ----------
    sequence_predictions: dict
        A dictionary to fill during the evaluation, which would later be used for visualization purposes.
    EU_countries_dict: dict
        A dictionary mapping between the names of the EU countries included
        in the empirical analysis and the index of the columns in the time series dataframe.

    Returns:
    ----------
    timeSeries_benchmark_results: pd.DataFrame
        Pandas dataframe containing the results from the evaluation of all models.
    """
    ...
    for current_model in models:
        with open('./evaluation/Results/'+current_model+'_Results_per_Subset.pkl','rb') as f:
            ...
            for subsets_idx in all_subsets_idx:
                ...
                if type(current_results[subsets_idx][1])!=np.float64:
                    true_batches=current_results[subsets_idx][1]
    ...
    with open('./evaluation/Results/TimesFM_Results_per_Subset.pkl','rb') as f:
        current_results_TimesFM=pickle.load(f)

    with open('./evaluation/Results/Chronos_Results_per_Subset.pkl','rb') as f:
        current_results_Chronos=pickle.load(f)

    with open('./evaluation/Results/ARIMA_Results_per_Subset.pkl','rb') as f:
